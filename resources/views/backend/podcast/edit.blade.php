    ...
    <div class="main-content-inner">
        <div class="main-content-wrap">
            <div class="flex items-center flex-wrap justify-between gap20 mb-27">
                <h3>Edit Podcasts Information</h3>
                <ul class="breadcrumbs flex items-center flex-wrap justify-start gap10">
                    ...
                    <li><a href="#">
                            <div class="text-tiny">Podcasts</div>
                        </a></li>
                    <li><i class="icon-chevron-right"></i></li>
                    <li>
                        <div class="text-tiny">Edit Podcasts </div>
                    </li>
                </ul>
            </div>

            <div class="wg-box">
                <form class="form-new-brand form-style-1" id="podcast-form" action="{{ route('podcast.update', $podcast->id) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <a class="tf-button style-1 w208" style="padding-left: 75px;"
                        href="{{ route('podcast.list') }}">Back</a>
                    ...
                    <fieldset class="name">
                        ...
                        <select class="flex-grow" id="category_id" name="category_id" required>
                            ...
                            @foreach ($all_categories as $category)
                                <option
                                    value="{{ $category->id }}"{{ $podcast->category_id == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow" id="sub_category" type="text" placeholder="Enter Sub Category Name" name="sub_category"
                            value="{{ $podcast->sub_category }}" required>
                    </fieldset>

                    <fieldset class="name">
                        <div class="body-title">Category Slug <span class="tf-color-1">*</span></div>
                        <input class="flex-grow" id="categorySlug" type="text" placeholder="Category Name" name="slug"
                            tabindex="0" value="{{ $podcast->slug }}" aria-required="true" required readonly>
                    </fieldset>
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow" type="text" placeholder="Video URL" name="video_url" tabindex="0"
                            value="{{ old('video_url', $podcast->video_url) }}" aria-required="true">
                    </fieldset>
                    ...
                    <fieldset>
                        ...
                        <div class="upload-image flex-grow">
                            ...
                            <div class="item" id="imgpreview" style="text-align: center;">
                                @if ($podcast->images)
                                    <img src="{{ asset('storage/uploads/backend/podcast/' . $podcast->images) }}" id="preview-img"
                    ...
                    <fieldset class="name">
                        ...
                        <textarea class="flex-grow" style="height:90px;" type="text" placeholder="Short Description" name="short_desc"
                            id="short_desc" tabindex="0" aria-required="true" value="{{ $podcast->short_desc }}">{{ $podcast->short_desc }}
                        </textarea>
                    </fieldset>

                    <div class="form-group">
                        <label><strong>Description :</strong></label>
                        <textarea class="summernote" name="description" value="{{ $podcast->description }}" id="description">{!! $podcast->description !!}</textarea>
                    </div>
        ...
        $('#podcast-form').on('submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var formDataObj = $('#podcast-form').serializeArray();
                    ...
                    $.ajax({
                        url: "{{ route('podcast.update', $podcast->id) }}",
                        method: "POST",
                        data: formData,
