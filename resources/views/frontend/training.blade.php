            ...
            <div class="container">
                <div class="row">
                                ...
                                <div class="ecsp_div">
                                    <ul class="tab2">
                                        ...
                                        @foreach ($groupOrder as $group)
                                            @if (!empty($categoriesGrouped[$group]))
                                                ...
                                                <ul id="group-{{ Str::slug($group) }}" class="group-categories"
                                                    style="display: none;">
                                                    @foreach ($categoriesGrouped[$group] as $category)
                                                        <li id="li_border">
                                        ...
                                        @if (!empty($categoriesGrouped['']))
                                            @foreach ($categoriesGrouped[''] as $category)
                                                <li id="li_border">
                                                    <a class="tablinks1 {{ !$firstCategorySet ? 'active' : '' }}"
                                                        href="#{{ Str::slug($category->name) }}"
                                                        onclick="openCity(event, '{{ Str::slug($category->name) }}')">
                    ...
                    <div class="col-sm-6 col-md-9">
                        <div id="code-content">
                            @foreach ($categoriesGrouped as $groupName => $groupedCategories)
                                @foreach ($groupedCategories as $category)
                                    <div id="{{ Str::slug($category->name) }}" class="tabcontent1"
                                        style="display:{{ !$loop->parent->index && !$loop->index ? 'block' : 'none' }};">
                                        <div class="mrgn-brdr">
                                                        ...
                                                        <div class="iconbox">
                                                            ...
                                                            <div class="box-content" id="clr-learn">
                                                                ...
                                                                <span class="box-readmore">
                                                                    <a
                                                                        href="{{ route('l3-template', [
                                                                            'sb' => $training->subcategory->id ?? 0,
                                                                            'pg' => $page_id,
                                                                            'ct' => $category->id,
                                                                        ]) }}">
            ...
            @foreach ($orderedCategories as $index => $category)
                <div class="m-container" id="m-{{ Str::slug($category->name) }}">
                    ...
                    @if ($categoryTrainings->count())
                                    ...
                                    @foreach ($trainingChunk as $singleTraining)
                                        <div class="item">
                                            <div class="mrgn-btm-iconbx">
                                                <a
                                                    href="{{ route('l3-template', [
                                                        'sb' => $singleTraining->subcategory->id,
                                                        'pg' => $page_id,
                                                        'ct' => $category->id,
                                                    ]) }}">
                                                    <div class="iconbox">
                        ...
                        @endforeach
                    @else
                        <div class="no-training text-muted p-3">
